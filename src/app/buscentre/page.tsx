...
export default function MyBuscentrePage() {
  ...
  const actingParam       = actingBuscentreId ? `?actingBuscentreId=${actingBuscentreId}` : "";

  async function load() {
    setLoading(true);
    const [ovRes, cellsRes] = await Promise.all([
      fetch(`/api/buscentre/overview${actingParam}`),
      fetch(`/api/buscentre/cells${actingParam}`),
    ]);
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-6 max-w-[1200px] mx-auto pb-20 lg:pb-8">
      ...
      <AttendanceStatsSection
        ...
        actingParam={activeView?.isActing && activeView.buscentreId ? `actingBuscentreId=${activeView.buscentreId}` : ""}
      />
      ...
      {cells.length === 0 ? (
        ...
        <div className="flex flex-col gap-3 mb-6">
          {cells.map((cell) => <CellCard key={cell.id} cell={cell} onRefresh={load} />)}
        </div>
      )}
